...
public class ViewDragHelper {
    ...
    /* JADX WARN: Code restructure failed: missing block: B:45:0x00f6, code lost:            if (r2 != r15) goto L44;     */
    /*
    ...
    */
    ...
    public boolean checkTouchSlop(int directions, int pointerId) {
        ...
        float dx = fArr2[pointerId] - fArr4[pointerId];
        float dy = fArr3[pointerId] - fArr[pointerId];
        if (!checkHorizontal || !checkVertical) {
            return checkHorizontal ? Math.abs(dx) > ((float) this.mTouchSlop) : checkVertical && Math.abs(dy) > ((float) this.mTouchSlop);
        }
        float f = (dx * dx) + (dy * dy);
    ...
    public boolean isEdgeTouched(int edges, int pointerId) {
        int[] iArr;
        return (!isPointerDown(pointerId) || (iArr = this.mInitialEdgesTouched) == null || (iArr[pointerId] & edges) == 0) ? false : true;
    }
