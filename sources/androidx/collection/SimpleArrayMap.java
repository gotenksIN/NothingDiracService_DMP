...
public class SimpleArrayMap<K, V> {
    ...
    public SimpleArrayMap(int capacity) {
        this.hashes = capacity == 0 ? ContainerHelpers.EMPTY_INTS : new int[capacity];
        this.array = capacity == 0 ? ContainerHelpers.EMPTY_OBJECTS : new Object[capacity << 1];
    }
    ...
    private final int indexOf(K key, int hash) {
        ...
        int index = ContainerHelpers.binarySearch(this.hashes, n, hash);
        if (index < 0) {
            return index;
        }
    ...
    private final int indexOfNull() {
        ...
        int index = ContainerHelpers.binarySearch(this.hashes, n, 0);
        if (index < 0) {
            return index;
        }
    ...
    public V put(K key, V value) {
        ...
        if (i3 < i) {
            ...
            ArraysKt.copyInto(objArr2, objArr2, (i3 + 1) << 1, i3 << 1, this.size << 1);
        }
        int i5 = this.size;
        if (i == i5) {
    ...
    public V removeAt(int index) {
        ...
        if (i <= 1) {
            ...
            if (iArr.length > 8 && i < iArr.length / 3) {
                ...
                if (index > 0) {
                    ArraysKt.copyInto(iArr2, this.hashes, 0, 0, index);
