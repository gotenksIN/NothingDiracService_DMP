...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m2317plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m2228toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != Long.MAX_VALUE) {
            long j = this.reading;
            newReading = j + longDelta;
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m2316overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m2225toDoubleimpl(duration, getUnit());
    ...
    private final void m2316overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m2231toStringimpl(duration)) + '.');
    }
}
