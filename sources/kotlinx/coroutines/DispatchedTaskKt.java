...
public final class DispatchedTaskKt {
    ...
    public static final <T> void dispatch(DispatchedTask<? super T> dispatchedTask, int mode) {
        ...
        Continuation delegate = dispatchedTask.getDelegate$external__kotlinx_coroutines__android_common__kotlinx_coroutines();
        ...
        if (!undispatched && (delegate instanceof DispatchedContinuation) && isCancellableMode(mode) == isCancellableMode(dispatchedTask.resumeMode)) {
            ...
            if (dispatcher.isDispatchNeeded(context)) {
                dispatcher.mo2403dispatch(context, dispatchedTask);
                return;
            } else {
    ...
    public static final <T> void resume(DispatchedTask<? super T> dispatchedTask, Continuation<? super T> delegate, boolean undispatched) {
        UndispatchedCoroutine undispatchedCompletion$iv$iv;
        Intrinsics.checkNotNullParameter(dispatchedTask, "<this>");
        Intrinsics.checkNotNullParameter(delegate, "delegate");
        Object state = dispatchedTask.takeState$external__kotlinx_coroutines__android_common__kotlinx_coroutines();
        Throwable exception = dispatchedTask.getExceptionalResult$external__kotlinx_coroutines__android_common__kotlinx_coroutines(state);
        Result.Companion companion = Result.INSTANCE;
        Object result = Result.m856constructorimpl(exception != null ? ResultKt.createFailure(exception) : dispatchedTask.getSuccessfulResult$external__kotlinx_coroutines__android_common__kotlinx_coroutines(state));
        if (!undispatched) {
    ...
    private static final void resumeUnconfined(DispatchedTask<?> dispatchedTask) {
        EventLoop eventLoop = ThreadLocalEventLoop.INSTANCE.getEventLoop$external__kotlinx_coroutines__android_common__kotlinx_coroutines();
    ...
    public static final void runUnconfinedEventLoop(DispatchedTask<?> dispatchedTask, EventLoop eventLoop, Function0<Unit> block) {
        Intrinsics.checkNotNullParameter(dispatchedTask, "<this>");
    ...
    public static final void resumeWithStackTrace(Continuation<?> continuation, Throwable exception) {
        ...
        continuation.resumeWith(Result.m856constructorimpl(ResultKt.createFailure(th)));
    }
}
